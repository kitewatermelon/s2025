import numpy as np
import torch
from torch import autograd, linspace, nn, sigmoid, sqrt
from torch.special import expm1
from tqdm import trange

from models.lvdm.utils import unsqueeze_right


class VDM(nn.Module):
    def __init__(self, model, cfg, ae, image_shape):
        super().__init__()
        self.model = model  # UViT
        self.cfg = cfg
        self.image_shape = image_shape
        self.vocab_size = 1
        self.ae = ae
        # center_slice_only=True  → 2D 모드: recon_loss를 가운데 슬라이스만으로 계산
        # center_slice_only=False → 2.5D 모드: 전체 슬라이스 평균으로 계산
        self.center_slice_only = getattr(cfg, "center_slice_only", False)
    ...
    def recon_loss(self, img, f):
        ...
        z0_r = z0 / alpha0
        with torch.no_grad():
            x_dist = self.ae.decode_stage_2_outputs(z0_r)
            if self.center_slice_only:
                mid = x_dist.shape[2] // 2
                x_dist = x_dist[:, :, mid, :, :]
                img = img[:, :, mid, :, :]
    ...
    def forward(self, x, cond, img, *, noise=None):
        ...
        pred_loss = snr_t * ((x_hat - x) ** 2).sum((1, 2, 3))
