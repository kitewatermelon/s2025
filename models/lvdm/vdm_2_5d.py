...
class VDM(nn.Module):
    ...
    def sample_p_s_t(self, z, t, s, clip_samples, context=None, eta: float=0.0):
        ...
        # DEBUG
        # print("[sample_p_s_t, z_hat]",z_hat.shape)
        if clip_samples:
    ...
    def recon_loss(self, img, f):
        ...
        with torch.no_grad():
            ...
            # print("x_dist:", x_dist.shape, "img:", img.shape)
            
            x_dist = x_dist[:, :, 2, :, :]  # 가운데 슬라이스 (index 2)
            img = img[:, :, 2, :, :]  # 가운데 슬라이스 (index 2)
            # print("x_dist:", x_dist.shape, "img:", img.shape)
    ...
    def forward(self, x, cond, img, *, noise=None):
        # print(x.shape[1:] , self.image_shape)
