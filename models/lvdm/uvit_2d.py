...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # DEBUG
        """
            dimension before embeding patch
            [PatchEmbed, x1] torch.Size([6, 2, 32, 32])
            dimension after embeding patch
            [PatchEmbed, x2] torch.Size([6, 256, 512])
        """
        # print("dimension before embeding patch")        
        # print("[PatchEmbed, x1]", x.shape)
        x = self.proj(x).flatten(2).transpose(1, 2)
        # print("dimension after embeding patch")
        # print("[PatchEmbed, x2]", x.shape)
        # DEBUG END
        #pdb.set_trace()
        return x
...
class UViT(nn.Module):
    ...
    def forward(self, x, timesteps, context=None):
        ...
        # DEBUG
        """     
            before concat x, time_token
            [UViT, time_token] torch.Size([6, 1, 512])
            [UViT, x] torch.Size([6, 256, 512])
            after concat x, time_token
            [UViT, x] torch.Size([6, 257, 512])
        """
        # print("before concat x, time_token")
        # print("[UViT, time_token]", time_token.shape)
        # print("[UViT, x]", x.shape)
        x = torch.cat((time_token, x), dim=1)
        # print("after concat x, time_token")
        # print("[UViT, x]", x.shape)
        # DEBUG END
