...
def main(config):
    ...
    for epoch in epbar:
        ...
        for batch_idx, batch_data in enumerate(bbar):
            ...
            # fig, axs = plt.subplots(1, 5, figsize=(15, 3))  # 1행 5열
            # axs[0].imshow(images[0].squeeze(0).cpu().numpy(), cmap='gray')
            # axs[0].set_title(f"Slice {0}")
            # axs[0].axis('off')

            # plt.tight_layout()
            # plt.savefig("slices.png")  # 이미지 파일로 저장
            # plt.show()                 # 화면에 출력
