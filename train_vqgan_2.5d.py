    ...
    for epoch in epbar:
        ...
        for batch_idx, batch_data in enumerate(bbar):
            ...
            with torch.amp.autocast("cuda", enabled=config["optim"]["amp"], dtype=torch.bfloat16):
                ...
                recon_2d = recon[:, :, 2, :, :]  # 가운데 슬라이스 (index 2)
                data_2d = data[:, :, 2, :, :]  # 가운데 슬라이스 (index 2)
                # print(recon_2d.shape, data_2d.shape)
        ...
        if epoch % val_every == 0:
            ...
            with torch.no_grad():
                for val_step, batch_data in enumerate(tqdm(val_loader, desc="Validating", leave=False)):
                    ...
                    if val_step == 0:
                        B, C, H, W = data_2d.shape

                        data_axial = data_2d[...].float()
                        recon_axial = recon_2d[...].float()
